$(document).ready(function() {
	...
	$.ajaxSetup({
	   ...
	   $(document).on('change', '.compraCheck', function(){
		var comprado = $(this).is(':checked');
		var urls = "/combo/catForm/{{$id}}"; // Verifique a estrutura correta da URL
		jQuery.ajax({
			url: urls,
			method: 'get',
			data: {
				checkAdq: comprado // Enviar o valor do checkbox corretamente
			},
			success: function(result){
				$("#DivFormComplemento").html(result);
				$(".select2").select2();
				// Atualize o estado do checkbox com base na resposta do servidor
				$("#checkAdq").prop("checked", comprado);
			}
		});
	});
